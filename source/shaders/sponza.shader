...
void main() {
    ...
    DefaultVertexRef vertex_ref = g_vs_push_constants.vertices_ref[index_ref.index];
    MeshRef          mesh_ref   = g_vs_push_constants.meshes_ref[gl_BaseInstance];
    ...
    gl_Position      = g_per_view_uniforms[g_vs_push_constants.per_view_uniform_index].view_proj_matrix * f32vec4(ws_position, 1.0);
...
layout(push_constant) uniform PushConstants {
    layout(offset = 32) MaterialRef materials_ref;
                        uint32_t    per_view_uniform_index;
} g_ps_push_constants;
